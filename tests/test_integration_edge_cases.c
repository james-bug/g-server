...
void test_cec_device_failure_handling(void) {
    ...
    // === 場景 2: 初始化成功但運行中斷線 ===
    printf("\n  Scenario 2: Device disconnected during operation...\n");
    ...
    // 查詢狀態失敗
    cec_monitor_query_state_IgnoreAndReturn(CEC_ERROR_DEVICE_NOT_FOUND);
    ps5_power_state_t state;
    int query_ret = cec_monitor_query_state(&state);
    TEST_ASSERT_EQUAL(CEC_ERROR_DEVICE_NOT_FOUND, query_ret);
    printf("    ✗ Query failed - device disconnected\n");
    ...
    // === 場景 4: 使用快取狀態作為降級方案 ===
    printf("\n  Scenario 4: Fallback to cached state...\n");
...
void test_websocket_abnormal_connections(void) {
    ...
    // 先建立滿額的連線（假設上限 10）
    for (int i = 1; i <= 10; i++) {
        ws_server_get_client_count_ExpectAndReturn(i);
    }
    ...
    // 模擬客戶端 5 異常斷線
    printf("    Client 5 disconnected abnormally\n");
...
void test_ps5_wake_retry_and_timeout(void) {
    ...
    ps5_wake_with_retry_ExpectAndReturn(&ps5_info, 3, 30, WAKE_RESULT_FAILED);
    wake_result_t result1 = ps5_wake_with_retry(&ps5_info, 3, 30);
    TEST_ASSERT_EQUAL(WAKE_RESULT_FAILED, result1);
    printf("    Attempt 1: FAILED\n");
    printf("    Attempt 2: FAILED\n");
    printf("    Attempt 3: FAILED\n");
    printf("    ✗ Wake failed after all retries\n");
