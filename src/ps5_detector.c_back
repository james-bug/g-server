...
bool ps5_detector_validate_ip(const char *ip) {
    ...
    // Simple IP validation: X.X.X.X format
    int dots = 0;
    int digits = 0;
...
static int load_cache_from_file(ps5_info_t *info) {
    ...
    // Copy data
    strncpy(info->ip, ip_item->valuestring, PS5_IP_MAX_LEN - 1);
    strncpy(info->mac, mac_item->valuestring, PS5_MAC_MAX_LEN - 1);
...
static int check_arp_table(ps5_info_t *info) {
    ...
    // Read ARP table
    snprintf(cmd, sizeof(cmd), "cat /proc/net/arp 2>/dev/null || arp -a 2>/dev/null");
    ...
    while (line != NULL) {
        ...
        if (sscanf(line, "%15s %*s %*s %17s", ip, mac) == 2) {
            if (ps5_detector_validate_ip(ip) && ps5_detector_validate_mac(mac)) {
                ...
                strncpy(info->ip, ip, PS5_IP_MAX_LEN - 1);
                strncpy(info->mac, mac, PS5_MAC_MAX_LEN - 1);
        ...
        if (strstr(line, "Nmap scan report for") != NULL) {
            ...
            if (ip_start != NULL) {
                ...
                if (ps5_detector_validate_ip(ip_start)) {
                    strncpy(info->ip, ip_start, PS5_IP_MAX_LEN - 1);
                    info->last_seen = time(NULL);
                    info->online = true;
    ...
    if (result == PS5_DETECT_OK) {
        ...
        if (info->mac[0] == '\0') {
            ...
            if (check_arp_table(&arp_info) == PS5_DETECT_OK) {
                strncpy(info->mac, arp_info.mac, PS5_MAC_MAX_LEN - 1);
            }
        }
